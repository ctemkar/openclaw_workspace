...
btc_stop_triggered = btc_price <= btc_stop
eth_stop_triggered = eth_price <= eth_stop

# Check for take-profit triggers
btc_take_triggered = btc_price >= btc_take
eth_take_triggered = eth_price >= eth_take

# Critical drawdown indicators (loss > 4.5%)
btc_critical = btc_pl_pct <= -4.5
eth_critical = eth_pl_pct <= -4.5
...
print(f'Timestamp: {local_time} (Asia/Bangkok) / {utc_time} UTC')
print(f'')
print(f'=== MARKET DATA ===')
...
print(f'  Stop-Loss Triggered: {"YES 🚨" if btc_stop_triggered else "No"}')
print(f'  Take-Profit Triggered: {"YES ✅" if btc_take_triggered else "No"}')
print(f'  Critical Drawdown: {"YES ⚠️" if btc_critical else "No"}')
print(f'')
print(f'ETH/USD POSITION:')
...
print(f'  Stop-Loss Triggered: {"YES 🚨" if eth_stop_triggered else "No"}')
print(f'  Take-Profit Triggered: {"YES ✅" if eth_take_triggered else "No"}')
print(f'  Critical Drawdown: {"YES ⚠️" if eth_critical else "No"}')
print(f'')
...
print(f'=== ALERTS ===')
alerts = []
if btc_stop_triggered:
    alerts.append(f'🚨 BTC STOP-LOSS TRIGGERED at ${btc_stop:.2f} (current: ${btc_price})')
if eth_stop_triggered:
    alerts.append(f'🚨 ETH STOP-LOSS TRIGGERED at ${eth_stop:.2f} (current: ${eth_price})')
if btc_take_triggered:
    alerts.append(f'✅ BTC TAKE-PROFIT TRIGGERED at ${btc_take:.2f} (current: ${btc_price})')
if eth_take_triggered:
    alerts.append(f'✅ ETH TAKE-PROFIT TRIGGERED at ${eth_take:.2f} (current: ${eth_price})')
if btc_critical:
    alerts.append(f'⚠️ BTC CRITICAL DRAWDOWN: {btc_pl_pct:.2f}% loss')
if eth_critical:
    alerts.append(f'⚠️ ETH CRITICAL DRAWDOWN: {eth_pl_pct:.2f}% loss')
...
if btc_stop_triggered or eth_stop_triggered:
    print('1. EXECUTE STOP-LOSS ORDERS IMMEDIATELY')
    print('2. Review trading strategy and risk parameters')
    print('3. Consider reducing position sizes')
elif btc_critical or eth_critical:
    print('1. Consider tightening stop-loss levels')
    print('2. Monitor positions closely for further deterioration')
    print('3. Prepare for potential stop-loss execution')
elif overall_risk == 'EXTREME' or overall_risk == 'HIGH':
    print('1. Monitor positions closely')
    print('2. Consider setting price alerts near stop-loss levels')
    print('3. Review market conditions for potential trend changes')
else:
    print('1. Continue monitoring positions')
    print('2. Maintain current risk parameters')
print(f'')
print(f'=== END OF MONITORING LOG ===')