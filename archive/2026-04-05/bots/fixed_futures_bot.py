#!/usr/bin/env python3
"""
FIXED BINANCE FUTURES SHORT TRADING BOT
With proper API calls for Binance Futures
"""
...
# Setup logging
log_file = os.path.join(BASE_DIR, "fixed_futures_trading.log")
logging.basicConfig(
    level=logging.INFO,
...
# Cryptocurrencies (filter problematic ones)
FUTURES_CRYPTOS = [
...
# Trading parameters
SHORT_THRESHOLD = 1.0   # 1.0% drop
SCAN_INTERVAL = 120      # 120 seconds
...
def execute_futures_short(exchange, analysis, usdt_balance):
    ...
    try:
        ...
        try:
            ...
            # For Binance Futures, we need to use specific endpoint
            # Try setting leverage via params
            params = {'leverage': LEVERAGE}
            
            logger.warning(f"📉 PLACING REAL SHORT ORDER: {amount:.6f} {symbol} @ market")
            
            # For SHORT in ONE_WAY mode, we use 'sell' side
            order = exchange.create_order(
                symbol=symbol,
                type='market',
                side='sell',  # 'sell' for SHORT in ONE_WAY mode
                amount=amount,
                params=params
            )
...
def main():
    ...
    logger.critical("=" * 70)
    logger.critical("🚀 FIXED BINANCE FUTURES SHORT TRADING BOT")
    logger.critical("=" * 70)
    logger.critical("⚠️  REAL TRADE EXECUTION ENABLED")
    logger.critical("=" * 70)
    ...
    try:
        while True:
            ...
            logger.info(f"\n📊 TRADING CYCLE {cycle} - {cycle_start.strftime('%H:%M:%S')}")
            logger.info("-" * 50)
            ...
            if trades_today >= MAX_DAILY_TRADES:
                logger.warning(f"⚠️  Daily trade limit reached: {trades_today}/{MAX_DAILY_TRADES}")
            else:
                # Check futures pairs
                ...
                for crypto in FUTURES_CRYPTOS:
                    ...
                    if analysis and analysis['signal'] == 'SHORT':
                        ...
                        if free_balance >= 10 and trades_today < MAX_DAILY_TRADES:
                            # EXECUTE REAL TRADE
                            trade_result = execute_futures_short(exchange, analysis, free_balance)
                            if trade_result and trade_result.get('status') == 'EXECUTED':
                                trades_executed += 1
                                trades_today += 1
                                
                                # Stop after one trade for safety
                                break
                        else:
                            logger.warning(f"⚠️  Cannot trade: Balance=${free_balance:.2f}, Daily={trades_today}/{MAX_DAILY_TRADES}")
