#!/usr/bin/env python3
"""
FIXED GEMINI BOT - Uses LIMIT orders for Gemini
- Fixes "allows limit orders only" error
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('fixed_gemini_trades.log'),
        logging.StreamHandler()
    ]
)

class FixedGeminiTrader:
    def __init__(self):
        logging.info("🚀 FIXED GEMINI TRADER STARTING")
        logging.info("💰 FIXED: Uses LIMIT orders for Gemini")
    ...
    def execute_trade_fixed(self, opp):
        """Execute trade with FIXED Gemini LIMIT orders"""
        logging.info(f"🎯 EXECUTING FIXED TRADE: {opp['crypto']}")
        ...
        try:
            ...
            if opp['buy_exchange'] == 'gemini':
                # Gemini: Use LIMIT buy order
                buy_symbol = f"{crypto}/USD"
                buy_price = opp['buy_price'] * 1.001  # 0.1% above for quick fill
                buy_order = self.gemini.create_limit_buy_order(buy_symbol, buy_amount, buy_price)
                logging.info(f"   ✅ LIMIT BUY at ${buy_price:.4f}")
            else:
                # Binance: Use MARKET buy order
                buy_symbol = f"{crypto}/USDT"
                buy_order = self.binance.create_market_buy_order(buy_symbol, buy_amount)
                logging.info(f"   ✅ MARKET BUY at ${opp['buy_price']:.4f}")
            ...
            if opp['sell_exchange'] == 'gemini':
                # Gemini: Use LIMIT sell order
                sell_symbol = f"{crypto}/USD"
                sell_price = opp['sell_price'] * 0.999  # 0.1% below for quick fill
                sell_order = self.gemini.create_limit_sell_order(sell_symbol, buy_amount, sell_price)
                logging.info(f"   ✅ LIMIT SELL at ${sell_price:.4f}")
            else:
                # Binance: Use MARKET sell order
                sell_symbol = f"{crypto}/USDT"
                sell_order = self.binance.create_market_sell_order(sell_symbol, buy_amount)
                logging.info(f"   ✅ MARKET SELL at ${opp['sell_price']:.4f}")
            ...
            logging.info(f"   🎉 FIXED TRADE COMPLETE!")
    ...
    def run(self):
        """Main loop"""
        logging.info("=" * 60)
        logging.info("🚀 FIXED GEMINI TRADER - LIMIT ORDERS")
        logging.info("=" * 60)
        ...
        while True:
            try:
                scan_count += 1
                logging.info(f"🔍 Fixed Scan #{scan_count}")
                ...
                logging.info("🛑 Fixed trader stopped")
                break
            except Exception as e:
                logging.error(f"❌ Error: {e}")
                time.sleep(60)
