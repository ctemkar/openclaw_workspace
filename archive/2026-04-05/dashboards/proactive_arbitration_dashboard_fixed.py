...
def dashboard():
    ...
    <body>
        <div class="container">
            ...
            <div class="stats">
                <div class="stat-item">
                    <div class="stat-value">''' + str(system_status['running_systems']) + '''/''' + str(system_status['total_systems']) + '''</div>
                    <div class="stat-label">Systems Running</div>
                </div>
                <div class="stat-item">
                    <div class="stat-value">$''' + f"{system_status['total_profit']:.2f}" + '''</div>
                    <div class="stat-label">Crypto Profit</div>
                </div>
                <div class="stat-item">
                    <div class="stat-value">$''' + f"{system_status['forex_balance']:.2f}" + '''</div>
        ...
        html += f'''
                    <div class="system-card">
                        ...
                        <div class="system-status {'running' if 'RUNNING' in system['status'] else 'stopped'}">
                            {system['status']}
                            {f"<span style='color: #94a3b8; font-size: 0.9em;'> (PID: {system['pid']})</span>" if 'pid' in system else ''}
                        </div>
                        <div style="margin: 10px 0; color: #cbd5e1;">{system['description']}</div>
        '''
        
        if 'total_profit' in system:
            html += f'''<div class="profit">💰 Profit: ${system['total_profit']:.2f}</div>'''
        
        if 'balance' in system:
            html += f'''<div class="profit" style="color: #fbbf24;">💰 Balance: ${system['balance']:.2f}</div>'''
        
        if 'total_trades' in system:
            html += f'''<div style="color: #94a3b8;">📊 Trades: {system['total_trades']}</div>'''
        
        if 'capital' in system:
            html += f'''<div style="color: #94a3b8;">💼 Capital: {system['capital']}</div>'''
