...
CRYPTOS = ['BTC', 'ETH', 'SOL', 'XRP', 'ADA', 'DOT', 'DOGE', 'AVAX', 'LINK', 'UNI', 'LTC', 'ATOM', 'FIL', 'XTZ', 'AAVE', 'COMP', 'YFI', 'BAT', 'SUSHI', 'CRV', 'SHIB', 'BCH', 'PEPE', 'WIF', 'BONK', 'FLOKI', 'INJ', 'MATIC', 'BNB', 'TRX', 'ETC', 'ALGO', 'NEAR', 'MKR', 'SNX', 'ZRX', 'ENJ', '1INCH', 'UMA', 'MEME', 'ARB', 'OP', 'IMX', 'METIS', 'RNDR', 'FET', 'AGIX', 'OCEAN', 'SAND', 'MANA', 'AXS', 'GALA', 'EOS', 'XLM', 'VET', 'THETA']  # 56 cryptos (removed unsupported on Gemini)  # 50 cryptos for maximum arbitrage opportunities  # 27 supported cryptos (removed unsupported)  # 34 cryptos for maximum arbitrage coverage

# Trading parameters
MIN_SPREAD_PERCENT = 0.4      # 0.5% minimum spread to trade
MIN_PROFIT_AMOUNT = 0.15  # Lowered to make 0.4% spreads profitable       # Minimum $0.25 profit after fees
TRADE_SIZE_PERCENT = 0.08     # 8% of available capital per trade (AGGRESSIVE)
MAX_TRADE_SIZE = 200.0        # Maximum $200 per trade (AGGRESSIVE)
...
class AutoArbitrageBot:
    ...
    def calculate_arbitrage_opportunity(self, crypto: str) -> Optional[Dict]:
        ...
        logger.debug(f"DEBUG {crypto}: gemini=${gemini_price:.6f}, binance=${binance_price:.6f}")
        
        if gemini_price <= 0 or binance_price <= 0:
            logger.debug(f"DEBUG {crypto}: Price check failed - gemini=${gemini_price:.6f}, binance=${binance_price:.6f}")
            return None
    ...
    def monitor_and_trade(self):
        ...
        while True:
            ...
            try:
                ...
                top_cryptos = CRYPTOS  # Log all cryptos  # Log top 3
