...
def load_trading_config():
    # Construct the path to trading_config.json relative to the current script's directory
    script_dir = os.path.dirname(os.path.abspath(__file__))
    config_path = os.path.join(script_dir, "..", "trading_config.json")
    
    default_config = {
        "capital": 10000.0,
        "trade_size_usd": 10.00, # Default to $10
        "stop_loss_pct": 0.01,
    ...
    if os.path.exists(config_path):
        try:
            with open(config_path, 'r') as f:
                config = json.load(f)
                # Ensure all keys are present, use defaults if missing
...
def call_gemini_api(prompt_text, investment_usd):
    try: 
        return {"model": "gemini-pro", "strategy_name": "Gemini_Momentum_Strategy", "description": "Identifies upward momentum and suggests buying dips below a certain threshold.", "symbol": "BTC/USD", "investment_usd": investment_usd, "risk_parameters": {"stop_loss_pct": 0.05, "take_profit_pct": 0.10}, "profit_rationale": "Based on historical trends showing strong bounce-backs after minor pullbacks.", "llm_provider": "gemini"}
    except Exception as e: print(f"Error calling Gemini API (placeholder): {e}"); return None

def call_claude_api(prompt_text, investment_usd):
    try: 
        return {"model": "claude-3-haiku-20240307", "strategy_name": "Claude_Sentiment_Strategy", "description": "Analyzes recent news sentiment and suggests trades based on overwhelmingly positive or negative sentiment shifts.", "symbol": "ETH/USD", "investment_usd": investment_usd, "risk_parameters": {"stop_loss_pct": 0.07, "take_profit_pct": 0.12}, "profit_rationale": "Positive sentiment historically correlates with short-term price increases.", "llm_provider": "claude"}
    except Exception as e: print(f"Error calling Claude API (placeholder): {e}"); return None

def call_openai_api(prompt_text, investment_usd):
    try: 
        return {"model": "gpt-4o", "strategy_name": "OpenAI_Volatility_Strategy", "description": "Targets short-term volatility by scalping small price movements in highly liquid assets.", "symbol": "SOL/USD", "investment_usd": investment_usd, "risk_parameters": {"stop_loss_pct": 0.04, "take_profit_pct": 0.08}, "profit_rationale": "Exploits intraday price fluctuations common in volatile markets.", "llm_provider": "openai"}
    except Exception as e: print(f"Error calling OpenAI API (placeholder): {e}"); return None

def call_deepseek_api(prompt_text, investment_usd):
    try: 
        return {"model": "deepseek-coder", "strategy_name": "DeepSeek_Momentum_Scalp_Strategy", "description": "Focuses on quick, small gains from strong short-term momentum indicators.", "symbol": "BTC/USD", "investment_usd": investment_usd, "risk_parameters": {"stop_loss_pct": 0.02, "take_profit_pct": 0.03}, "profit_rationale": "Scalping relies on frequent, small profits to accumulate over time.", "llm_provider": "deepseek"}
    except Exception as e: print(f"Error calling DeepSeek API (placeholder): {e}"); return None

def call_qwen_api(prompt_text, investment_usd):
    try: 
        return {"model": "qwen-turbo", "strategy_name": "Qwen_Trend_Following_Strategy", "description": "Identifies established trends and aims to follow them for larger gains.", "symbol": "ETH/USD", "investment_usd": investment_usd, "risk_parameters": {"stop_loss_pct": 0.10, "take_profit_pct": 0.20}, "profit_rationale": "Riding established trends can lead to significant profits, accepting larger drawdowns.", "llm_provider": "qwen"}
...
def get_llm_strategies(num_btc_strategies=5):
    ...
    investment_usd_from_config = trading_config.get("trade_size_usd", 10.00) # Use loaded config value

    print(f"Using trade size from config: ${investment_usd_from_config:.2f}")

    print("Generating LLM strategies...")

    gemini_btc = call_gemini_api(btc_prompt, investment_usd_from_config)
    if gemini_btc: all_strategies.append(gemini_btc)
    deepseek_btc = call_deepseek_api(btc_prompt, investment_usd_from_config)
    if deepseek_btc: all_strategies.append(deepseek_btc)
        
    claude_eth = call_claude_api(other_cryptos_prompt, investment_usd_from_config)
    if claude_eth: all_strategies.append(claude_eth)
        
    qwen_sol = call_qwen_api(other_cryptos_prompt, investment_usd_from_config)
    if qwen_sol: all_strategies.append(qwen_sol)

    openai_suggested = call_openai_api(suggested_crypto_prompt, investment_usd_from_config)
...
if __name__ == "__main__":
    ...
    # Ensure the config file is in the same directory as THIS script for loading purposes
    # Alternatively, adjust path based on where trading_config.json is expected relative to this script.
    # For now, assuming it's in the parent directory of the script's location.
