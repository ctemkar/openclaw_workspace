#!/usr/bin/env python3
"""
Cleanup ALL duplicate cron jobs monitoring localhost:5001
"""
...
def main():
    print("🚨 MASSIVE CLEANUP: Disabling ALL duplicate cron jobs...")
    ...
    print(f"\nFound {len(duplicate_jobs)} duplicate trading dashboard monitor jobs to disable")
    ...
    for job in jobs:
        name = job.get("name", "")
        if "Progress Report" in name or "auto_save" in name or "progress_monitor" in name or "Crypto Trading LLM Bot" in name:
            essential_jobs.append(job)
    
    print(f"\nKeeping {len(essential_jobs)} essential jobs:")
    for job in essential_jobs:
        print(f"  - {job.get('name', 'unnamed')} ({'enabled' if job.get('enabled') else 'disabled'})")
