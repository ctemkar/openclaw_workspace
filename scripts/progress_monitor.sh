...
if ps aux | grep -v grep | grep -q "practical_profit_bot.py"; then
    ...
    if tail -5 logs/practical_profit_output.log 2>/dev/null | grep -q "Invalid API-key"; then
        echo "[ERROR] BINANCE API ERROR: Invalid API key/permissions"
        echo "   Status: BOT RUNNING BUT NOT TRADING"
        echo "   Last trade: $(tail -1 logs/practical_profits.log 2>/dev/null | cut -d' ' -f1-2 || echo 'NO TRADES TODAY')"
        echo "   Today's profit: $0.00 (NO TRADES)"
    else
        # Check TODAY'S profits only
        if [ -f "logs/practical_profits.log" ]; then
            echo ""
            ...
            TODAYS_TRADES=$(grep "^$TODAY" logs/practical_profits.log 2>/dev/null)
            ...
            if [ -n "$TODAYS_TRADES" ]; then
                ...
                echo "   Last trade: $(tail -1 logs/practical_profits.log 2>/dev/null | cut -d' ' -f1-3 || echo 'NONE')"
            fi
        fi
    fi
else
...
if [ -f "logs/real_26_crypto_arbitrage.log" ]; then
    echo "   • Top spreads from arbitrage bot:"
    # Get the latest spread data
    LATEST_SPREADS=$(tail -50 logs/real_26_crypto_arbitrage.log 2>/dev/null | grep -A15 "TOP 10 CRYPTO SPREADS" | head -16)
    ...
    if [ -n "$LATEST_SPREADS" ]; then
        ...
        # Show best opportunity
        BEST_OPP=$(tail -50 logs/real_26_crypto_arbitrage.log 2>/dev/null | grep -A2 "Best opportunity:" | head -3)
        if [ -n "$BEST_OPP" ]; then
